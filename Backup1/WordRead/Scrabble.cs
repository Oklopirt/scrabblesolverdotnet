 ...
 * Date: 6/11/2009
 * Time: 8:44 PM
 * TODO: dunno, everything seems fine...
 *
 */
using System;
...
namespace WordRead
{
	...
	public double getAll( ref List<wdescrip> wordListGet){
       ...
       // object Lock = new object();
        // var cmdList = new[] { new { row = 0, col = 0, dir = RIGHT } }.ToList();
        //cmdList.Clear();

      //  List<wdescrip> wordList = new List<wdescrip>(Lock);

        List<wdescrip> horizontals = new List<wdescrip>();
        List<wdescrip> verticals = new List<wdescrip>();

        ThreadStart horizontalThread = () =>
        {
                ...
                for (int j = 0; j < board.GetLength(1) - 1; j++)
                {
                    ...
                    if (first_letter(i, j, RIGHT) || (board[i, j] == EMPTY && (j == 0 || board[i, j - 1] == EMPTY)))
                        if (firstword || (!firstword && i == 7))
                            // cmdList.Add(new { row = i, col = j, dir = RIGHT });
                            RowCol(horizontals, i, j, RIGHT);

                }
            }
        };

        ThreadStart verticalThread = () =>
        {
                ...
                for (int j = 0; j < board.GetLength(1); j++)
                {
                    if (first_letter(i, j, DOWN) || (board[i, j] == EMPTY && (i == 0 || board[i - 1, j] == EMPTY)))
                        if (firstword || (!firstword && j == 7))
                            // cmdList.Add(new { row = i, col = j, dir = DOWN });
                            RowCol(verticals, i, j, DOWN);
                }
            }
        };

        var horizontalsT = new Thread(horizontalThread);
        var verticalsT = new Thread(verticalThread);
        ...
        /*foreach (var cmd in cmdList)
            RowCol(wordList, cmd.col, cmd.row, cmd.dir);*/

       

        if (!firstword)
        {
            horizontals.RemoveAll(x => x.Dir == Scrabble.RIGHT && !(x.Col <= 7 && (x.Col + x.Length - 1) >= 7));
            verticals.RemoveAll(x => x.Dir == Scrabble.DOWN && !(x.Row <= 7 && (x.Row + x.Length - 1) >= 7));
        }
        horizontals.AddRange(verticals);
        wordListGet = horizontals;
        wordListGet.Sort();
	                      ...
	                      int startRow, int startCol, int curRow, int curCol, bool dir,bool bridge, bool linkedin,bool sevenLet, int baseValue,int extraVal, int multiplier)
	{
		...
		if(board[curRow,curCol] != Scrabble.EMPTY){
			...
			if(dir == Scrabble.RIGHT){
				...
				if(bridge && ((curCol == board.GetLength(1)-1) || (board[curRow,curCol+1] == Scrabble.EMPTY))){ //|| ((board[curRow,curCol+1] == Scrabble.EMPTY) && bridge)){
				...
				if(bridge && ((curRow == board.GetLength(0)-1) || (board[curRow+1,curCol] == Scrabble.EMPTY))){ // || ((board[curRow+1,curCol] == Scrabble.EMPTY) && bridge) ){
		...
		//the letter space is not occupied
			...
			foreach(char useLetter in availLetters){
				...
				if (has_neighbors(curRow,curCol,dir)){
					//Console.WriteLine(useLet + " has neighbors");
				    link = true;
	...
	private string aroundletters(char letter, int row, int col, bool dir){
		...
		if(dir == Scrabble.RIGHT){
			...
			string s = "";
			for(int i = firstrow; i<board.GetLength(0) && board[i,col]!=EMPTY; i++){
				//Console.WriteLine("Adding " + board[i,col]);
				s = s + board[i,col];}
			...
			string s = "";
			for(int i = firstcol; i<board.GetLength(1) && board[row,i]!=EMPTY; i++){
				s = s + board[row,i];}
